...
runcommand "$ENGINE -i settings/load-flat.sql -f ddl-tpch/text/alltables.sql --hivevar DB=tpch_text_${SCALE} --hivevar LOCATION=${DIR}/${SCALE}"
...
do
  ...
  COMMAND="$ENGINE -i settings/load-partitioned.sql -f ddl-tpch/bin_partitioned/${tbl}.sql \
    --hivevar DB=${DATABASE} \
    --hivevar SCALE=${SCALE} \
...
if [ "X$HUDI" != "X" ]; then
  runcommand "$ENGINE -f ddl-tpch/bin_partitioned/analyze_hudi.sql --hivevar DB=${DATABASE} --hivevar REDUCERS=${REDUCERS}"
else
  runcommand "$ENGINE -f ddl-tpch/bin_partitioned/analyze.sql --hivevar DB=${DATABASE} --hivevar REDUCERS=${REDUCERS}"
fi
